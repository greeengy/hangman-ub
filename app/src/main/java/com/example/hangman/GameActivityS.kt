...
class GameActivityS : AppCompatActivity() {

    private val gameManagerS = GameManagerS()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val gameState = gameManagerS.startNewGame()
        updateUI(gameState)

        lettersLayout.children.forEach { letterView ->
            if (letterView is TextView) {
                letterView.setOnClickListener {
                    val gameState = gameManagerS.play((letterView).text[0])
                    updateUI(gameState)
